"""Figure 15: arrows between 2016 and terminal-year UF levels."""
...
CONFIG = {
    ...
    "sheet": "graficos_ufs",
    "output_stem": "fig_15_trajetoria_renda_custo_ufs",
    "data_file": FIGURE_DATA_DIR / "fig_15_state_trajectories.csv",
    "title": "Figura 15. UFs: trajetória da renda e do custo da criminalidade",
...
def prepare_data() -> list[dict[str, str]]:
    ...
    if end_year <= start_year:
        raise ValueError(f"Figure 15 needs an end year after {start_year}; found {end_year}.")
    ...
    if keys != expected:
        raise ValueError(f"Figure 15 UF-year coverage mismatch: missing={sorted(expected - keys)}")
