"""Figure 14: cross-UF burden and composition in the latest complete year."""
...
CONFIG = {
    ...
    "sheet": "graficos_ufs",
    "output_stem": "fig_14_custos_economicos_ufs",
    "data_file": FIGURE_DATA_DIR / "fig_14_state_costs.csv",
    "title": "Figura 14. UFs: nível e composição dos custos econômicos da criminalidade",
...
def prepare_data() -> list[dict[str, str]]:
    ...
    if {str(record["uf"]) for record in source} != UF_CODES:
        raise ValueError(f"Figure 14 does not contain all 27 UFs in {ano_final}.")
