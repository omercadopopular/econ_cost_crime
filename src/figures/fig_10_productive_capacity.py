"""Figure 10: modeled loss of productive capacity due to homicides."""
...
CONFIG = {
    ...
    "output_stem": "fig_10_perda_capacidade_produtiva",
    "data_file": FIGURE_DATA_DIR / "fig_10_productive_capacity.csv",
    "title": "Figura 10. Brasil: perda de capacidade produtiva por homicídios",
...
def prepare_data() -> list[dict[str, str]]:
    ...
    for year in sorted(source):
        ...
        for row in component_rows:
            row["total_de_homicidios"] = require_number(
                source[year]["total_de_homicidios"], context=f"homicide count {year}"
            )
        rows.extend(component_rows)
    write_csv(CONFIG["data_file"], rows, (*NATIONAL_LONG_FIELDS, "total_de_homicidios"))
